...
-- comment grey; everything else = fg. functions = warm hero, types + preproc =
-- cool, strings + escape = green, numbers/const + Error = rose, keywords = plum-
-- grey; operators/punct recede down a neutral grey ladder. No bg on code (only
...
return function(p, cfg)
    ...
    h.WinSeparator    = { fg = p.grey, bg = BG }
    h.VertSplit       = { fg = p.grey, bg = BG }
    ...
    h.PmenuKind       = { fg = p.role_type, bg = BG }
    h.PmenuKindSel    = { fg = p.role_type, bg = p.sel, bold = true }
    ...
    -- Reference the palette's role_* keys (palette.lua owns the hex). Keyed by
    -- hex VALUE: each key MUST be a distinct color or Lua silently merges rows
    -- (the @number-goes-purple bug). Share a color by putting groups in the SAME
    -- row (Error rides const's row), never by repeating a key.
    local fg_by_color = {
        [p.role_fn] = { -- HERO: functions — call-site landmark
            "Function", "@function", "@method",
        },
        [p.role_type] = {      -- types + scaffolding, one tone (blend)
            "Type",            -- legacy base: StorageClass/Structure/Typedef
            "@type",           -- typedefs, structs, size_t/u8/ssize_t
            "@type.builtin",   -- int/char/void — folded in (equal-lum split available)
            "@type.qualifier", -- const/volatile
            "@module", "@namespace",
            "@keyword.directive", "@keyword.directive.define", "@keyword.import",
            "Define", "PreProc", "@function.macro",
            "Constant", "@constant",
        },
        -- To split builtin types by an equal-luminance nudge: uncomment sibling
        -- `type_alt` in palette.lua and add  [p.role_type_alt] = { "@type.builtin" }.
        [p.role_keyword] = { -- ALL keywords, one tone (control flow folded in)
        ...
        [p.role_string] = { -- strings + escapes, one tone (blend)
            "String", "@string",
            "Character", "@character",
        ...
        [p.role_const] = { -- numbers / constants + syntax Error (all muted rose)
            "@number", "@boolean",
            "Error",
        },
        [p.role_op] = { -- operators — grey, just under the bone field
            "Operator", "@operator", "@punctuation.special",
        },
        [p.role_punct] = { -- punctuation / delimiters — a step dimmer
            "@punctuation", "@tag.delimiter", "Delimiter",
        },
        -- ---- BONE content family (value depth, no color) --------------------
        [p.fg] = { -- variables / identifiers — the key light
            "@variable", "Identifier", "@property"
        },
        [p.bone_param] = { -- parameters — inputs, one step down
            "@variable.parameter",
        },
        [p.bone_field] = { -- fields / members / properties — data on objects
            "@variable.member", "@field", "@constructor",
        },
    }
    -- NOTE: keyed by hex VALUE — each key MUST be a distinct color, or Lua
    -- silently merges the rows (that was the @number-goes-purple bug). Error
    -- shares const's ROW (same key), never its own key.
    for color, groups in pairs(fg_by_color) do
    ...
    set(comment_list, { fg = p.role_comment }) -- hue only, no italic

    h.Underlined                  = { fg = p.role_type, underline = true }
    h.Todo                        = { fg = p.bg, bg = p.role_fn, bold = true }
    h["@markup.heading"]          = { fg = p.role_fn, bold = true }
    h["@markup.link"]             = { fg = p.role_type, underline = true }
    ...
    h.TelescopeResultsTitle   = { fg = p.role_type, bg = "NONE" }
    ...
    h.SpellLocal              = { undercurl = true, sp = p.role_type }
